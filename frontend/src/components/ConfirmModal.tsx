        ...
        <div style={{
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                    <button className="btn btn-secondary" onClick={onCancel}>{cancelLabel || t('confirmModal.cancel')}</button>
                    <button ref={btnRef} className={danger ? 'btn btn-danger' : 'btn btn-primary'} onClick={onConfirm}>{confirmLabel || t('confirmModal.confirm')}</button>
                </div>
            </div>
        </div>
    );
